...
function clockStart(k){k=k||activeClock||'A';const c=clocks[k];if(!c)return;if(!c.running){c.running=true;c.stopped=false;c.start=Date.now();alertMsg('Clock '+k+' running')}saveClocks();renderClocks()}
function clockPause(k){k=k||activeClock||'A';const c=clocks[k];if(!c)return;if(c.running){c.elapsed+=Date.now()-c.start;c.running=false;c.start=0;c.stopped=false;alertMsg('Clock '+k+' paused')}saveClocks();renderClocks()}
function clockStop(k){k=k||activeClock||'A';const c=clocks[k];if(!c)return;const elapsedBefore=clockMs(k);if(c.running){c.elapsed+=Date.now()-c.start;}else{c.elapsed=elapsedBefore;}c.running=false;c.start=0;c.stopped=true;saveClocks();renderClocks();const timeEl=$('clock'+k+'Time')||$('activeClockTime');const moneyEl=$('clock'+k+'Money')||$('activeClockMoney');alertMsg('Clock '+k+' stopped at '+(timeEl?.textContent||'00:00:00')+' / '+(moneyEl?.textContent||'$0.00'));}
function clockReset(k){k=k||activeClock||'A';clocks[k]={elapsed:0,running:false,start:0,label:'',stopped:false};const label=$('clock'+k+'Label')||($('activeClockLabel')&&k===activeClock?$('activeClockLabel'):null);if(label)label.value='';saveClocks();renderClocks();alertMsg('Clock '+k+' cleared')}
...
function renderClocks(){
  ['A','B','C'].forEach(k=>{
    ...
    const status=clocks[k].stopped?'STOPPED':(clocks[k].running?'RUNNING':(ms?'PAUSED':'READY'));
