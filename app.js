const DEFAULTS={rate:135,service:250,tax:7,card:0,markup:15,rent:1500,insurance:145,electric:250,other:0,backend:'https://rolling-wrench-ai-backend.onrender.com',supabase:'https://uxpkqwcmvtqvubibbrek.supabase.co',supabaseKey:'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InV4cGtxd2NtdnViaWJicmVrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzcyMzk4NjQsImV4cCI6MjA5MjgxNTg2NH0.afiaSFqkRFEXW5nPQVRXKZcpKkS6iF3T_hTQC2P15HQ',theme:'green',mode:'shop'};
...
function setTheme(t){document.body.className=document.body.className.replace(/\b(green|orange|red|blue|night)\b/g,'').trim();document.body.classList.add(t||'green');const s=settings();s.theme=t||'green';store.set('settings',s)}
...
function docHeader(title){const c=store.get('customer',{}),t=store.get('truck',{});return `<h3>${title}</h3><b>Rolling Wrench Diesel LLC</b><br>James Jacobs • 260-502-6222<br>www.rollingwrenchdiesel.com<hr><b>Customer:</b> ${esc(c.name||'')} ${esc(c.phone||'')}<br><b>Truck:</b> ${esc([t.unit,t.year,t.make,t.model,t.engine].filter(Boolean).join(' '))}<br><b>VIN:</b> ${esc(t.vin||'')}<br><b>Date:</b> ${now()}<hr>`}
function buildQuote(){const x=calcTotals();$('quoteOut').innerHTML=docHeader('QUOTE')+`<b>Job:</b> ${esc($('quoteJob').value)}<br><b>Work:</b><br>${esc($('quoteDesc').value).replace(/\n/g,'<br>')}<hr>Labor (${x.h.toFixed(2)} hrs @ ${money(x.r)}): ${money(x.labor)}<br>Service Call: ${money(x.svc)}<br>Parts: ${money(x.p)}<br>Tax: ${money(x.tax)}<br>Card Fee: ${money(x.card)}<h3>Total: ${money(x.total)}</h3><small>Quote subject to inspection, parts availability, and customer approval.</small>`;store.set('lastQuote',x);updateFinance();alertMsg('Quote built')}
...
function buildInvoice(){const x=calcTotals(),copy=$('invoiceType').value,canvas=$('sigCanvas');let html=docHeader('INVOICE - '+copy)+`<b>Notes / Work:</b><br>${esc($('invoiceNotes').value||$('quoteDesc').value).replace(/\n/g,'<br>')}<hr>Labor: ${money(x.labor)}<br>Service Call: ${money(x.svc)}<br>Parts: ${money(x.p)}<br>Tax: ${money(x.tax)}<br>Card Fee: ${money(x.card)}<h3>Total Due: ${money(x.total)}</h3>`;if(!isCanvasBlank(canvas))html+=`<b>Customer Signature:</b><br><img src="${sigData()}" style="max-width:100%;border:1px solid #111">`;else html+='<b>Customer Signature:</b> NOT SIGNED';$('invoiceOut').innerHTML=html;store.set('lastInvoice',{total:x.total,date:now(),html});updateFinance();alertMsg('Invoice built')}
